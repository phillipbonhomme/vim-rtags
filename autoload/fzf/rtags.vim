function! fzf#rtags#get_filepath(result)
    return split(a:result, ':')[0]
endfunction

function! fzf#rtags#get_fileline(result)
    return split(a:result, ':')[1]
endfunction

function! fzf#rtags#get_filecol(result)
    return split(a:result, ':')[2]
endfunction

function! fzf#rtags#get_filetext(result)
    return substitute( a:result, '^[^:]\+:[^:]\+:[^:]\+:\t', '', '')
endfunction

function! fzf#rtags#get_word(result)
    let cwd = getcwd()
